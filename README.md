...
I've spent 3 years building systems where correctness actually matters —
payment gateways, transaction flows, webhook handlers, and the kind of 
backend work where a bug isn't just a bug, it's a financial loss.
...
- Fuel card system — a closed-loop payment platform for a logistics 
  company where fleet owners issue prepaid fuel cards to their drivers, 
  topped up via payment gateways and redeemable only at fuel stations

## Currently building
Failure-Resilient Payment Processing System — A payment processing system focused on failure — duplicate webhooks, 
failed retries, double charges. The edge cases that break most systems. 
